...
function Bridge () {
    ...
    const sourceTokenBalance = async () => {
    ...
    const destTokenBalance = async () => {
        const provider = new ethers.JsonRpcProvider(CHAINS_DATA[toChain].rpc);
        const dummyERC20 = new ethers.Contract(CHAINS_DATA[toChain]["fpv"], DummyERC20ABI, provider);
        const decimals = await dummyERC20.decimals()
        const _balance = await dummyERC20.balanceOf(address)
        const formattedBalance = ethers.formatUnits(_balance, decimals)
    ...
    const checkAllowance = async (_amount) => {
        ...
        const dummyERC20 = new ethers.Contract(CHAINS_DATA[chain.id]["fpv"], DummyERC20ABI, provider);
        const decimals = await dummyERC20.decimals()
        const allowance = await dummyERC20.allowance(address, ownerWallet.address.toString())
    ...
    const initiateBridge = async () => {
        if(tokenAmount > 0) {
            ...
            const dummyERC20From = new ethers.Contract(CHAINS_DATA[chain.id]["fpv"], DummyERC20ABI, providerFrom);
            const dummyERC20FromSigner = dummyERC20From.connect(signerFrom)

            const dummyERC20To = new ethers.Contract(CHAINS_DATA[chain.id]["fpv"], DummyERC20ABI, providerTo);
            const dummyERC20ToSigner = dummyERC20To.connect(signerTo)

            const formattedBalance = ethers.formatUnits(tokenAmount, dummyERC20FromSigner.decimals())

            await dummyERC20FromSigner.transferFrom(address, signerFrom.address, formattedBalance)

            await dummyERC20FromSigner.burnSupply(formattedBalance)

            await dummyERC20ToSigner.mintSupply(formattedBalance)
        }
    }
